...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	protected Object doGetTransaction() {
		//管理connection对象,创建会滚点,按照会滚点回滚,释放会滚点
		//DataSourceTransactionObject 事物对象
		DataSourceTransactionObject txObject = new DataSourceTransactionObject();
		//DataSourceTransactionManager默认是允许嵌套事物的
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			if (!txObject.hasConnectionHolder() ||
				...
				//包装成 ConnectionHolder ,然后设置到事物对象
			...
			//设置只读事物,从设置的时间点开始 到事物结束过程中  其他事务所提交的数据，该事务将看不见！
			...
			if (txObject.isNewConnectionHolder()) {
				//如果是新建事物,则建立当前线程和数据库的连接关系
				//思路:从连接池 对象拿到 连接 ,然后 把事物对象的连接绑定上
